...
export function isDayClosed(date: string): boolean {
  const row = db
    .prepare("SELECT status FROM cash_closures WHERE business_date = ?")
...
export function getOrCreateCashClosure(
  ...
  db.prepare(
    `INSERT INTO cash_closures (id, business_date, opened_at, status, opening_balance, total_sales, total_collections, total_expenses, expected_cash, difference)
...
export function recalculateInvoiceBalance(
  ...
  const totalPaid = db
    .prepare(
      `SELECT COALESCE(SUM(amount),0) as total FROM financial_transactions WHERE invoice_id = ? AND type = 'PAYMENT' AND status = 'ACTIVE'`,
    )
  ...
  ).run(paid, remaining, status, new Date().toISOString(), invoiceId);
}

export function getFinancialSummary(date: string) {
  const sales = db
    .prepare(
      `SELECT COALESCE(SUM(total),0) as total FROM invoices WHERE invoice_date = ? AND status <> 'CANCELLED'`,
    )
    .get(date) as { total: number };
  const collections = db
    .prepare(
      `SELECT COALESCE(SUM(amount),0) as total FROM financial_transactions WHERE transaction_date = ? AND type = 'PAYMENT' AND status = 'ACTIVE'`,
    )
    .get(date) as { total: number };
  const expenses = db
    .prepare(
      `SELECT COALESCE(SUM(amount),0) as total FROM financial_transactions WHERE transaction_date = ? AND type = 'EXPENSE' AND status = 'ACTIVE'`,
    )
    .get(date) as { total: number };
  const oldDebtCollection = db
    .prepare(
      `SELECT COALESCE(SUM(t.amount),0) as total
       ...
       WHERE t.transaction_date = ? AND t.type = 'PAYMENT' AND t.status = 'ACTIVE'
         AND i.invoice_date <> ?`,
    )
...
export function getAllClosures(): Array<Record<string, unknown>> {
  return db
    .prepare("SELECT * FROM cash_closures ORDER BY business_date DESC")
...
export function getOutstandingDebts(): Array<Record<string, unknown>> {
  return db
    .prepare(
      `SELECT i.id, i.invoice_number, i.invoice_date, i.total, i.paid, i.remaining,
              c.name as customer_name, c.phone as customer_phone
       FROM invoices i
       JOIN customers c ON c.id = i.customer_id
       WHERE i.status IN ('UNPAID','PARTIALLY_PAID') AND i.status <> 'CANCELLED'
...
export function getCustomerStatement(
  ...
  const invoices = db
    .prepare(
      `SELECT invoice_date as date, invoice_number as ref, 'فاتورة' as type, total as debit, 0 as credit
  ...
  const payments = db
    .prepare(
      `SELECT t.transaction_date as date, t.transaction_number as ref, 'دفعة' as type, 0 as debit, t.amount as credit
       FROM financial_transactions t
       WHERE t.customer_id = ? AND t.type = 'PAYMENT' AND t.status = 'ACTIVE'`,
    )
    .all(customerId) as Array<{
      date: string;
      ref: string;
...
export function getDailyDashboardStats(date: string) {
  ...
  const totalDebts = db
    .prepare(
      `SELECT COALESCE(SUM(remaining),0) as total FROM invoices WHERE status IN ('UNPAID','PARTIALLY_PAID') AND status <> 'CANCELLED'`,
    )
    .get() as { total: number };
  return {
    ...summary,
